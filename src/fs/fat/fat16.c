...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != JOSHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while(1)
    {
        if( diskstreamer_read(stream, &item, sizeof(item)) != JOSHOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private , struct fat_directory* directory)
{
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if( diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != JOSHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if( diskstreamer_read(stream, dir, root_dir_size) != JOSHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if(diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != JOSHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if(fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != JOSHOS_ALL_OK)
    {   
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * JOSHOS_FAT16_FAT_ENTRY_SIZE));
    if(res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for(int i = 0; i < clusters_ahead; i++)
    {
        ...
        if(entry == JOSHOS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[JOSHOS_MAX_PATH];
