...
{
    int len = strnlen(filename, JOSHOS_MAX_PATH);
...
static int pathparser_get_drive_by_path(const char** path)
{
    ...
    //Add 3 bytes to slip drive number 0:/ 1:/ 2:/
    *path += 3;
    return drive_no;
}
...
{
    char* result_path_part = kzalloc(JOSHOS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if(strlen(path) > JOSHOS_MAX_PATH)
    {
        goto out;
    }
